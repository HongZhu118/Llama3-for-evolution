...
class SatellkiteCSP(Problem):
    ...
    def __init__(self, in_optimization_problem, debug=True) -> None:
        ...
        super(SatellkiteCSP, self).__init__(
    ...
    def compute(self, pop: Population) -> None:
                                ...
                                for m in range(len(start_time_range)):
                                    for n in range(m + 1, len(start_time_range)):
                                        ...
                                        if a1 <= b2 and b1 >= a2:
                                            ...
                                            return 100000, dict(sorted(task_time_dict.items(), key=lambda x: (x[1][0], x[1][1])))
                                start_time = min_range[0]
                                end_time = start_time + x[i]
                            for key, value in pre_task_dict.items():
